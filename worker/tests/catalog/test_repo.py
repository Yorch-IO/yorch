...
def catalog(database_url: str):
    ...
    with Catalog(database_url, min_size=1, max_size=2) as c:
        c.ensure_library("lib_1", "Biblioteca")
        yield c
...
def test_costs_accumulate_rather_than_overwrite(catalog: Catalog):
    ...
    catalog.start_run(run_id="run_1", workflow_id="wf-1", kind="index")
    catalog.record_cost(
...
def test_run_history_and_cost_survive_the_document_they_were_spent_on(
    ...
    catalog.start_run(run_id="run_1", workflow_id="wf_1", kind="index",
                      document_id="doc_1")
    catalog.attach_version("run_1", "doc_1", "ver_1")
    catalog.record_cost(
...
def test_removal_refuses_a_document_in_another_library(catalog: Catalog):
    ...
    _document(catalog)
    catalog.ensure_library("lib_2", "Otra")
...
def test_project_totals_count_a_shared_version_once(catalog):
    ...
    catalog.ensure_library("lib_totals", "Totales", "es")
...
def test_project_totals_count_only_what_was_activated(catalog):
    ...
    catalog.ensure_library("lib_totals2", "Totales", "es")
    ...
    assert catalog.project_totals().indexed_versions == 0
...
def test_pages_are_measured_as_absent_rather_than_assumed(catalog):
    ...
    catalog.ensure_library("lib_pages", "Páginas", "es")
    doc = catalog.upsert_document(
    ...
    assert catalog.project_totals().versions_with_pages == 0
...
def test_recent_runs_are_newest_first_and_bounded(catalog):
    catalog.ensure_library("lib_runs", "Runs", "es")
    doc = catalog.upsert_document(
...
def test_a_run_survives_the_document_it_was_spent_on(catalog):
    ...
    catalog.ensure_library("lib_orphan", "Huérfano", "es")
    doc = catalog.upsert_document(
    ...
    rows = [r for r in catalog.recent_runs(50) if r.workflow_id == "wf_o1"]
    assert len(rows) == 1
